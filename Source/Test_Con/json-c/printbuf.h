 ...
 */
extern int
printbuf_memappend(struct printbuf *p, const wchar_t *buf, int size);

#define printbuf_memappend_fast(p, bufptr, bufsize)          \
do {                                                         \
  if ((p->size - p->bpos) > bufsize) {                       \
    memcpy(p->buf + p->bpos, (bufptr), (bufsize) * sizeof(*p->buf));             \
    p->bpos += bufsize;                                      \
    p->buf[p->bpos]= '\0';                                   \
...
extern int
sprintbuf(struct printbuf *p, const wchar_t *msg, ...);
