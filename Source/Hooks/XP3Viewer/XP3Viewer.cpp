...
SET_GUID(CLSID_WICImagingFactoryX,  0xcacaf262, 0x9370, 0x4615, 0xa1, 0x3b, 0x9f, 0x55, 0x39, 0xda, 0x4c, 0xa);

HRESULT GetWICFactory(IWICImagingFactory** WICFactory)
{
    return CoCreateInstance(
                CLSID_WICImagingFactoryX,
                NULL,
...
PVOID FindWICConvert32bppBGRTo32bppBGRAAddress1(PVOID WindowsCodec)
{
    PIMAGE_DOS_HEADER dos = (PIMAGE_DOS_HEADER)WindowsCodec;
    PIMAGE_NT_HEADERS nt = (PIMAGE_NT_HEADERS)PtrAdd(dos, dos->e_lfanew);

    BYTE or_eax_ff000000[] = { 0x0d, 0x00, 0x00, 0x00, 0xff };
    BYTE or_esi_ff000000[] = { 0x81, 0xce, 0x00, 0x00, 0x00, 0xff };

    PVOID or = KMP(WindowsCodec, nt->OptionalHeader.SizeOfImage, or_eax_ff000000, sizeof(or_eax_ff000000));

    if (or == NULL)
        or = KMP(WindowsCodec, nt->OptionalHeader.SizeOfImage, or_esi_ff000000, sizeof(or_esi_ff000000));

    if (or == NULL)
    {
        or = (PVOID)IMAGE_INVALID_RVA;
    }
    else
    {
        or = PtrAdd(or, GetOpCodeSize(or) - 4);
    }
...
PVOID FindWICConvert32bppBGRTo32bppBGRAAddressSafe()
{
    PVOID WindowsCodec = NULL;

    SEH_TRY
    {
        WindowsCodec = Nt_LoadLibrary(L"WindowsCodecs.dll");

        if (WindowsCodec != NULL)
            return FindWICConvert32bppBGRTo32bppBGRAAddress1(WindowsCodec);
    }
    SEH_EXCEPT(EXCEPTION_EXECUTE_HANDLER)
    {
    }
...
VOID Init(HINSTANCE hInstance)
{
    ...
    if (IsKrkr2Exe)
    {
        ...
        WICConvert32bppBGRTo32bppBGRA = FindWICConvert32bppBGRTo32bppBGRAAddressSafe();

        MEMORY_PATCH p[] =
        {
            PATCH_MEMORY(0, 4, WICConvert32bppBGRTo32bppBGRA),
        };
    ...
    RTL_OSVERSIONINFOW f;

    f.dwOSVersionInfoSize = sizeof(f);
    RtlGetVersion(&f);
    ...
    if (f.dwMajorVersion == 6 && f.dwMinorVersion > 1 && IsWow64())
    {
        static WCHAR System32[] = L"System32\\";
        static WCHAR SysWow64[] = L"SysWow64\\";

        LdrModule = GetKernel32Ldr();

        CopyStruct(
            LdrModule->FullDllName.Buffer + (LdrModule->FullDllName.Length - LdrModule->BaseDllName.Length) / sizeof(WCHAR) - CONST_STRLEN(System32),
            SysWow64,
            CONST_STRLEN(SysWow64) * sizeof(WCHAR)
        );
    }
