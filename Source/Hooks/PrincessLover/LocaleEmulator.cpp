...
DWORD MByteToWChar(LPCSTR lpAnsiString, DWORD Length, LPWSTR lpUnicodeBuffer, DWORD BufferCount)
{
    ...
    if (Length == -1)
        Length = StrLengthA(lpAnsiString);
...
)
{
    ...
    CopyMemory(&NewTextMetricA, &lpntme->ntmTm, GetStructMemberOffset(NEWTEXTMETRICW, tmHeight, tmFirstChar));
    CopyMemory(
...
LONG_PTR WINAPI MySetWindowLongW(HWND hWnd, INT Index, LONG_PTR NewLong)
{
    ...
    switch (Index)
    {
        case GWL_WNDPROC:
            ...
            return (LONG_PTR)_InterlockedExchange((PLONG)&pInfo->OldWndProcA, NewLong);
    }
...
{
    return pInfo == NULL ? NULL : (pInfo->hHook = SetWindowsHookExA(WH_CBT, CBTProc, NULL, Nt_GetCurrentThreadId()));
}
...
GetInlineHookInfo(
    ...
    PVOID                   pAPIEntry,
    PVOID                   pNewRoutine,
    PVOID                   pAPIOldRoutine = NULL
)
{
    PBYTE pbApi;
    ULONG Instruction, Length, BytesNop;

    f.CallBack   = pAPIOldRoutine;
    f.NewFunction  = pNewRoutine;
    f.BytesToNop = 0;

    if (pAPIOldRoutine == NULL || pAPIEntry == NULL)
    {
        f.RVA = (ULONG_PTR)pAPIEntry;
        return;
    }

    BytesNop = 0;
    pbApi     = (PBYTE)pAPIEntry;
    while (BytesNop < 5)
    {
        Instruction = *(PDWORD)pbApi;
        if ((BYTE)Instruction == 0xEB)          // jmp short
        {
            pbApi += 2 + (LONG_PTR)(HIBYTE(Instruction));
            pAPIEntry = (PVOID)pbApi;
            continue;
        }
        else if ((WORD)Instruction == 0x25FF)        // jmp  dword ptr [const]
        {
            pbApi = (PBYTE)**(PULONG_PTR *)(pbApi + 2);
            pAPIEntry = pbApi;
            continue;
        }

        Length = GetOpCodeSize(pbApi);
        pbApi += Length;
        BytesNop += Length;
    }

    f.BytesToNop = BytesNop;

#if 0
    while (f.BytesToNop < 5)
    {
        Instruction = *(PDWORD)pbApi;
        if ((BYTE)Instruction == 0xEB)          // jmp short
        {
        ...
        else if ((BYTE)Instruction == 0x6A)     // push const < 0x80
        {
            pbApi += 2;
            f.BytesToNop += 2;
        }
        else if ((BYTE)Instruction >= 0xB8 && (BYTE)Instruction <= 0xBF)  // mov r32, const_32
        {
            pbApi += 5;
            f.BytesToNop += 5;
        }
        ...
        else if ((WORD)Instruction == 0x15FF)       // call dword ptr [const]
        {
            pbApi += 6;
            f.BytesToNop += 6;
        }
        else if ((WORD)Instruction == 0x3D83)       // cmp dword ptr [const], 0x7F
        {
            pbApi += 7;
            f.BytesToNop += 7;
        }
        else if ((WORD)Instruction == 0x3D81)       // cmp dword ptr [const], 0x80
        {
            pbApi += 0xA;
            f.BytesToNop += 0xA;
        }
...
MEMORY_FUNCTION_PATCH GetInlineHookInfo(PVOID pAPIEntry, PVOID pNewRoutine, PVOID pAPIOldRoutine = NULL)
{
    MEMORY_FUNCTION_PATCH f;
    GetInlineHookInfo(f, pAPIEntry, pNewRoutine, pAPIOldRoutine);
    return f;
}
...
/*
        msg.CsrClientConnect.ConnectionInfo     = (PVOID)0xB;
        msg.CsrClientConnect.ConnectionInfoSize = CodePage;

        CsrClientCallServer(&msg, NULL, 0x10010, 0xC);
        *pSectionHandle = (HANDLE)msg.CsrClientConnect.ServerId;
        Status = msg.Status;
        if (NT_SUCCESS(Status))
*/
            Status = 5;
...
// Locale Code Table
// http://msdn.microsoft.com/en-us/library/ms930130.aspx
...
//        g_TlsIndex = TlsIndex;
    ...
    MEMORY_PATCH p[] =
    {
        { CodePage, 4, (ULONG_PTR)&NlsAnsiCodePage },
        { TRUE,     1, (ULONG_PTR)&NlsMbCodePageTag },
    };
    ...
    switch (Version.dwMajorVersion)
    {
        ...
        case 6:
            ...
            pMByteToWChar = Nt_GetProcAddress(hModule, "MultiByteToWideChar");
            pWCharToMByte = Nt_GetProcAddress(hModule, "WideCharToMultiByte");
            break;
    ...
    {
        // gdi32
        INLINE_HOOK3(Nt_GetProcAddress(hModule, "GdiGetCodePage"), MyGdiGetCodePage),
        INLINE_HOOK3(EnumFontFamiliesA,            MyEnumFontFamiliesA),
        INLINE_HOOK3(EnumFontFamiliesExA,          MyEnumFontFamiliesExA),
        INLINE_HOOK3(CreateFontIndirectA,          MyCreateFontIndirectA),

        // user32
        INLINE_HOOK3(SetWindowTextA,               MySetWindowTextA),
        INLINE_HOOK3(GetWindowTextA,               MyGetWindowTextA),
        INLINE_HOOK3(CreateWindowExA,              MyCreateWindowExA),
        INLINE_HOOK3(DialogBoxParamA,              MyDialogBoxParamA),
        INLINE_HOOK3(CreateDialogParamA,           MyCreateDialogParamA),
        INLINE_HOOK3(DialogBoxIndirectParamA,      MyDialogBoxIndirectParamA),
        INLINE_HOOK3(CreateDialogIndirectParamA,   MyCreateDialogIndirectParamA),
        INLINE_HOOK2(CallWindowProcA,              MyCallWindowProcA,            OldCallWindowProcA),
        INLINE_HOOK3(DefWindowProcA,               MyDefWindowProcA),
        INLINE_HOOK3(DefMDIChildProcA,             MyDefMDIChildProcA),
        INLINE_HOOK3(DefDlgProcA,                  MyDefDlgProcA),
        INLINE_HOOK3(DefFrameProcA,                MyDefFrameProcA),
        INLINE_HOOK3(PostMessageA,                 MyPostMessageA),
        INLINE_HOOK3(SendMessageA,                 MySendMessageA),
        INLINE_HOOK3(SendNotifyMessageA,           MySendNotifyMessageA),
        INLINE_HOOK3(SendMessageCallbackA,         MySendMessageCallbackA),
        INLINE_HOOK3(SendMessageTimeoutA,          MySendMessageTimeoutA),
        INLINE_HOOK2(SetWindowLongA,               MySetWindowLongA,             OldSetWindowLongA),
        INLINE_HOOK2(GetWindowLongA,               MyGetWindowLongA,             OldGetWindowLongA),
        INLINE_HOOK2(SetWindowLongW,               MySetWindowLongW,             OldSetWindowLongW),
        INLINE_HOOK2(GetWindowLongW,               MyGetWindowLongW,             OldGetWindowLongW),
        INLINE_HOOK2(IsWindowUnicode,              MyIsWindowUnicode,            OldIsWindowUnicode),

        // kernel32
        INLINE_HOOK3(GetACP,                       MyGetACP),
        INLINE_HOOK2(GetSystemDefaultLangID,       MyGetSystemDefaultLangID,       OldGetSystemDefaultLangID),
        INLINE_HOOK2(pWCharToMByte,                MyWideCharToMultiByte,          OldWideCharToMultiByte),
        INLINE_HOOK2(pMByteToWChar,                MyMultiByteToWideChar,          OldMultiByteToWideChar),
        INLINE_HOOK3(IsDBCSLeadByte,               MyIsDBCSLeadByte),
        INLINE_HOOK3(CreateProcessInternalA,       MyCreateProcessInternalA),
        INLINE_HOOK2(CreateProcessInternalW,       MyCreateProcessInternalW,       OldCreateProcessInternalW),

        // ntdll
        INLINE_HOOK3(RtlMultiByteToUnicodeN,       MyRtlMultiByteToUnicodeN),
        INLINE_HOOK3(RtlUnicodeToMultiByteN,       MyRtlUnicodeToMultiByteN),
        INLINE_HOOK3(RtlMultiByteToUnicodeSize,    MyRtlMultiByteToUnicodeSize),
        INLINE_HOOK3(RtlUnicodeToMultiByteSize,    MyRtlUnicodeToMultiByteSize),
    };

//    for (DWORD i = countof(f), j = 0; i; ++j, --i)
//        GetInlineHookInfo(f[j], (PVOID)f[j].RVA, f[j].NewFunction, f[j].CallBack);

    Nt_PatchMemory(p, countof(p), f, countof(f), 0);
...
{
    if (g_pCodePageBase != NULL)
    {
        NTSTATUS Status;
        Status = NtUnmapViewOfSection(NtCurrentProcess(), g_pCodePageBase);
        if (NT_SUCCESS(Status))
            g_pCodePageBase = NULL;

        if (g_hHeap != NULL)
        {
            if (RtlDestroyHeap(g_hHeap) == NULL)
                g_hHeap = NULL;
        }
    }
