...
CED7AsDecompiler::CED7AsDecompiler()
{
...
VOID CED7AsDecompiler::ShowInstructionMapInfo()
{
    ULONG UnknownCount;
...
VOID CED7AsDecompiler::Reset(BOOL bDestroy /* = FALSE */)
{
...
VOID CED7AsDecompiler::ShowError(LONG Status)
{
...
LPSTR CED7AsDecompiler::AllocString(LPSTR pString, ULONG Length /* = -1 */)
{
    LPSTR pszBuffer, p;
...
BOOL CED7AsDecompiler::FreeString(PVOID pString)
{
...
LONG CED7AsDecompiler::AddInstrction(ED6_AS_CRAFT_INFO *pCraftInfo, ED6_INSTRUCTION *pInstrction)
{
...
BOOL CED7AsDecompiler::FindDecompiled(ULONG Offset, ED6_INSTRUCTION_RECORD &Decompiled)
{
    return FindMapWorder(m_DecompiledMap, Offset, Decompiled);
}

BOOL CED7AsDecompiler::AddDecompiled(ED6_INSTRUCTION_RECORD &Compiled)
{
    return AddMapWorder(m_DecompiledMap, Compiled);
}

BOOL CED7AsDecompiler::FindFunction(ULONG Offset, ED6_INSTRUCTION_RECORD &Function)
{
    return FindMapWorder(m_FuntionMap, Offset, Function);
}

BOOL CED7AsDecompiler::AddFunction(ED6_INSTRUCTION_RECORD &Function)
{
    return AddMapWorder(m_FuntionMap, Function);
}

BOOL CED7AsDecompiler::AddMapWorder(DecompiledInstructionMap &Map, ED6_INSTRUCTION_RECORD &Instruction)
{
...
BOOL CED7AsDecompiler::DecompilerFile(LPWSTR pszAsFileName, LPWSTR pszOutput /* = NULL */)
{
    LONG Status;
...
LONG CED7AsDecompiler::DecompilerFile(ED6_ACTION_SCRIPT_INFO *pAsInfo)
{
    LONG Status;
...
LONG CED7AsDecompiler::DecompileHeader(ED6_ACTION_SCRIPT_INFO *pAsInfo)
{
    PBYTE pbBuffer;
    ED6_AS_CRAFT_INFO *pCraftInfo;
    PUSHORT pCraftOffsetTable;
...
//        pCraftInfo->pInstruction = NULL;
    ...
    for (CHAR_CHIP_PATTERN_INFO *pChrPtn = pAsInfo->pChrChipPtnInfo; pChrPtn->ItcIndex != (USHORT)-1; ++pChrPtn)
...
LONG CED7AsDecompiler::DecompilerCrafts(ED6_ACTION_SCRIPT_INFO *pAsInfo)
{
    LONG Status;
    ED6_AS_CRAFT_INFO *pCraftInfo;

    pCraftInfo = pAsInfo->pCraftInfo;
    for (ULONG Count = pAsInfo->CraftCount; Count; ++pCraftInfo, --Count)
    {
        if (pCraftInfo->Offset == ED7_INVALID_OFFSET ||
            pCraftInfo->Offset <= pAsInfo->CraftOffsetTableOffset)
        {
            continue;
        }
...
LONG CED7AsDecompiler::DecompileFunctions(ED6_ACTION_SCRIPT_INFO *pAsInfo)
{
    LONG        Status;
    ULONG       FunctionCount;
    POSITION    Pos;
...
LONG CED7AsDecompiler::DecompilerOneCraft(ED6_ACTION_SCRIPT_INFO *pAsInfo, ED6_AS_CRAFT_INFO *pCraftInfo)
{
    LONG Status;
...
ED6_AS_INSTRUCTION_MAP* CED7AsDecompiler::FindInstrctionMap(PBYTE pbBuffer, PBYTE pbEnd)
{
    UNREFERENCED_PARAMETER(pbEnd);
    return FindInstrctionMapByCode(*pbBuffer);
}

ED6_AS_INSTRUCTION_MAP* CED7AsDecompiler::FindInstrctionMapByCode(ULONG Code)
{
    PVOID pMap;
    return m_InstructionMap.Lookup(Code, pMap) ? (ED6_AS_INSTRUCTION_MAP *)pMap : NULL;
}

LONG CED7AsDecompiler::WriteText(CFileDisk &file, LPSTR Buffer, LPCSTR pszFormat, ...)
{
    LONG Length;
    va_list args;
    CHAR szBuffer[0x400];
    ...
    return file.Write(szBuffer, Length) ? ASDECL_ERROR_SUCCESS : ASDECL_ERROR_WRITE_FILE;
}

#define WriteScr(...) { Status = WriteText(file, Buffer, __VA_ARGS__); AS_IF_FAIL_RETURN(Status); }

ED6_INSTRUCTION* CED7AsDecompiler::UnlinkInstruction(ED6_INSTRUCTION *pInstruction)
{
...
LONG CED7AsDecompiler::GetCraftName(ED6_ACTION_SCRIPT_INFO *pAsInfo, ULONG CraftIndex, LPSTR Buffer)
{
    LONG Length;
...
LONG CED7AsDecompiler::GetLabelName(ED6_ACTION_SCRIPT_INFO *pAsInfo, USHORT Offset, LPSTR Buffer)
{
...
LONG CED7AsDecompiler::DumpCrafts(ED6_ACTION_SCRIPT_INFO *pAsInfo, CFileDisk &file, LPSTR Buffer)
{
    LONG Status;
    PULONG pCraftIndex;
...
BOOL CED7AsDecompiler::IsLabelNextInstruction(ED6_INSTRUCTION *pInstruction, ULONG Offset)
{
...
)
{
    ...
    {
        pMap = FindInstrctionMapByCode(AS_GOTO);
...
LONG CED7AsDecompiler::CreateDefinition(LPWSTR pszAsFile, LPSTR Buffer)
{
...
LONG CED7AsDecompiler::DumpToFile(ED6_ACTION_SCRIPT_INFO *pAsInfo, LPWSTR pszAsFile, LPWSTR pszOutput)
{
    LONG        Status;
    CHAR        Buffer[0x400];
    CFileDisk   file;
    ...
    LOOP_ONCE
    {
        ...
        for (ULONG Index = 0, Count = pAsInfo->CraftCount; Count; ++pCraft, ++Index, --Count)
        {
            ...
            if (pCraft->Offset == ED7_INVALID_OFFSET ||
                pCraft->Offset <= pAsInfo->CraftOffsetTableOffset)
            {
                PBYTE p = (PBYTE)&pCraft->Offset;

                WriteScr(" [%02X", *p++);
                for (ULONG Len = sizeof(ED7_INVALID_OFFSET) - 1; Len; --Len)
                    WriteScr(" %02X", *p++);
                WriteScr("]");
