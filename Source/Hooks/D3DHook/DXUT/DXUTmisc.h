...
// A growable array
//--------------------------------------------------------------------------------------
template<typename TYPE> class DXUT_API CGrowableArray
{
public:
            CGrowableArray()
            {
            ...
            {
                for( ULONG i = 0; i < a.m_nSize; i++ ) Add( a.m_pData[i] );
            }
            ~CGrowableArray()
            {
                RemoveAll();
    ...
    const TYPE& operator[]( ULONG nIndex ) const
    {
        return GetAt( nIndex );
    }
    TYPE& operator[]( ULONG nIndex )
    {
        return GetAt( nIndex );
    }
    ...
    {
        if( this == &a ) return *this; RemoveAll(); for( ULONG i = 0; i < a.m_nSize;
                                                         i++ ) Add( a.m_pData[i] ); return *this;
    }

    HRESULT SetSize( ULONG nNewMaxSize );
    HRESULT Add( const TYPE& value );
    HRESULT Insert( ULONG nIndex, const TYPE& value );
    HRESULT SetAt( ULONG nIndex, const TYPE& value );
    TYPE& GetAt( ULONG nIndex ) const
    {
    ...
    ULONG     IndexOf( const TYPE& value )
    {
        return ( m_nSize > 0 ) ? IndexOf( value, 0, m_nSize ) : -1;
    }
    ULONG     IndexOf( const TYPE& value, ULONG iStart )
    {
        return IndexOf( value, iStart, m_nSize - iStart );
    }
    ULONG     IndexOf( const TYPE& value, ULONG nIndex, ULONG nNumElements );

    ULONG     LastIndexOf( const TYPE& value )
    {
        return ( m_nSize > 0 ) ? LastIndexOf( value, m_nSize - 1, m_nSize ) : -1;
    }
    ULONG     LastIndexOf( const TYPE& value, ULONG nIndex )
    {
        return LastIndexOf( value, nIndex, nIndex + 1 );
    }
    ULONG     LastIndexOf( const TYPE& value, ULONG nIndex, ULONG nNumElements );

    HRESULT Remove( ULONG nIndex );
    void    RemoveAll()
    {
        SetSize( 0 );
    }
...
protected:
    ...
    ULONG m_nSize;        // # of elements (upperBound - 1)
    ULONG m_nMaxSize;     // max allocated

    HRESULT SetSizeInternal( ULONG nNewMaxSize );  // This version doesn't call ctor or dtor.
};
...
template<typename TYPE> HRESULT CGrowableArray <TYPE>::SetSizeInternal( ULONG nNewMaxSize )
{
    ...
    if( nNewMaxSize == 0 )
    {
        ...
        if( m_pData )
        {
            MemoryAllocator::FreeMemory(m_pData);
            m_pData = NULL;
        }
    ...
    {
        // Grow array
        ULONG nGrowBy = ( m_nMaxSize == 0 ) ? 16 : m_nMaxSize;
        ...
        TYPE* pDataNew = ( TYPE* )MemoryAllocator::ReAllocateMemory( m_pData, nNewMaxSize * sizeof( TYPE ) );
        if( pDataNew == NULL )
...
//--------------------------------------------------------------------------------------
template<typename TYPE> HRESULT CGrowableArray <TYPE>::SetSize( ULONG nNewMaxSize )
{
    ULONG nOldSize = m_nSize;
    ...
    if( nOldSize > nNewMaxSize )
    {
        ...
        if( m_pData )
        {
            ...
            for( ULONG i = nNewMaxSize; i < nOldSize; ++i )
                m_pData[i].~TYPE();
        }
    }
    ...
    if( nOldSize < nNewMaxSize )
    {
        ...
        if( m_pData )
        {
            ...
            for( ULONG i = nOldSize; i < nNewMaxSize; ++i )
                new ( &m_pData[i] ) TYPE;
        }
    }
...
//--------------------------------------------------------------------------------------
template<typename TYPE> HRESULT CGrowableArray <TYPE>::Insert( ULONG nIndex, const TYPE& value )
{
    HRESULT hr;
...
//--------------------------------------------------------------------------------------
template<typename TYPE> HRESULT CGrowableArray <TYPE>::SetAt( ULONG nIndex, const TYPE& value )
{
    // Validate arguments
    if( nIndex < 0 ||
        nIndex >= m_nSize )
...
// section.
//--------------------------------------------------------------------------------------
template<typename TYPE> ULONG CGrowableArray <TYPE>::IndexOf( const TYPE& value, ULONG iStart, ULONG nNumElements )
{
    // Validate arguments
    if( iStart < 0 ||
        iStart >= m_nSize ||
    ...
    // Search
    for( ULONG i = iStart; i < ( iStart + nNumElements ); i++ )
    {
        if( value == m_pData[i] )
            return i;
    }
...
//--------------------------------------------------------------------------------------
template<typename TYPE> ULONG CGrowableArray <TYPE>::LastIndexOf( const TYPE& value, ULONG iEnd, ULONG nNumElements )
{
    // Validate arguments
    if( iEnd < 0 ||
        iEnd >= m_nSize ||
    ...
    // Search
    for( ULONG i = iEnd; i > ( iEnd - nNumElements ); i-- )
    {
        if( value == m_pData[i] )
            return i;
    }
...
//--------------------------------------------------------------------------------------
template<typename TYPE> HRESULT CGrowableArray <TYPE>::Remove( ULONG nIndex )
{
    if( nIndex < 0 ||
        nIndex >= m_nSize )
    {
