...
//        *(PDWORD)pbOutline2 = BitsLow;
    ...
    for (ULONG i = GlyphMetrics.gmBlackBoxY; i; --i)
    {
        ...
        CopyMemory(pbOutline, pbBuffer, BytesPerRowRaw / 4);
#else
...
BOOL FASTCALL LoadPlugins(PVOID Param1, PVOID Param2, PVOID Param3)
{
    ...
    INLINE_ASM mov EdiValue, edi;
    CopyMemory(GetACPBackup, GetACP, sizeof(GetACPBackup));
    {
        INTEL_STATIC MEMORY_FUNCTION_PATCH f[] =
        {
            { JUMP, (ULONG_PTR)GetACP, MyGetACP },
        };
        PatchMemory(NULL, 0, f, countof(f), NULL);
    }

    // ensure edi is not modified
    INLINE_ASM mov edi, EdiValue;
    ...
    hModule = LoadLibraryExW(L"DLL\\MainSystem.dll", NULL, 0);
    if (hModule != NULL)
    {
...
_Exit:
    ...
    if (VirtualProtectEx(
            ...
            &Old))
    {
        CopyMemory(GetACP, GetACPBackup, sizeof(GetACPBackup));
        VirtualProtectEx(NtCurrentProcess(), GetACP, 5, Old, &Old);
    }
...
//        { JUMP, (ULONG_PTR)PeekMessageA - (ULONG_PTR)hModule, MyPeekMessageA, 0, OldPeekMessageA },
    ...
    CopyMemory(GetACPBackup, GetACP, 5);
    ...
    LoadLibraryExW(L"DLL\\MainSystem.dll", NULL, 0);
