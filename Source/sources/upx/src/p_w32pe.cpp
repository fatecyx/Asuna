...
//    static const char exitp[] = "ExitProcess\x0\x0\x0";
    ...
    oimport = new upx_byte[soimport * 4];
    memset(oimport,0,soimport * 4);
...
/*
    ...
    for (ic = 0; ic < dllnum; ic++)
    {
        ...
        if (!*tarr)  // no imports from this dll
            continue;
#endif
        ppi = (unsigned char *)(((long)ppi + 3) & ~3);

        set_le32(ppi,idlls[ic]->npos);
        set_le32(ppi+4,idlls[ic]->iat - rvamin);
        ppi += 8;

        len = mbstowcs((wchar_t *)(ppi + 4), (char *)idlls[ic]->name, strlen(idlls[ic]->name) + 1);
        len *= sizeof(wchar_t);
        set_le16(ppi, len);
        set_le16(ppi + 2, len);
        ...
        for (; *tarr; tarr++)
        {
            ...
            {
                *ppi++ = 0xFF;
                set_le16(ppi, (unsigned short)*tarr);
                ppi += 2;
            }
            else
            {
                *ppi++ = 1;
                len = strlen(ibuf + *tarr + 2) + 1;
...
void PackW32Pe::buildLoader(const Filter *ft)
{
    ...
    if (soimport)
    {
        ...
        len = GetLoaderSize(upxloader_ProcessImport, upxloader_ProcessImportEnd);
        buf = linker->output + linker->outputlen;
        memcpy(buf, upxloader_ProcessImport, len);
        linker->outputlen += len;

        *(PULONG_PTR)&buf[2] = cimports;
    }
/*
    if (soimport)
        addLoader("PEIMPORT",
...
/*
    ...
    if (ih.entry)
    {
        ...
        len = GetLoaderSize(upxloader_JumpToOep, upxloader_JumpToOepEnd);
        buf = linker->output + linker->outputlen;
        memcpy(buf, upxloader_JumpToOep, len);
        linker->outputlen += len;
        ...
        vp_size = ((addr & 0xfff) + 0x28 >= 0x1000) ? 0x2000 : 0x1000;

        *(PULONG_PTR)&buf[2] = -vp_size;
        *(PULONG_PTR)&buf[8] = ih.entry;
    }
    else
    {
//        addLoader("PEMAIN20", NULL);
        addLoader("PERETURN", NULL);
    }
*/
...
//    else
//    {
//        PBYTE buf = (PBYTE)linker->output + linker->outputlen;
//        *(PUSHORT)buf = 0xE0FF; // jmp eax;
//        linker->outputlen += 2;
//    }
...
// info: see buildLoader()
    ...
    soimpdlls = 0;
    ODADDR(PEDIR_IMPORT) = 0;
    ODSIZE(PEDIR_IMPORT) = 0;
    processImports(ic, 0);
