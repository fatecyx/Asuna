...
command! -nargs=0 SessionOpenLast if exists('g:LAST_SESSION') | call s:OpenSession(g:LAST_SESSION) | else | call s:OpenSession('last_session') | endif
...
function! s:SessionAutoSave()
	if g:sessionman_save_on_exit != 0 && v:this_session != ''
		call s:SaveSession()
	elseif expand("%h") != ''
		call s:SaveSessionAs('last_session')
	endif
endfunction

aug sessionman
	au VimLeavePre * call s:SessionAutoSave()
aug END
