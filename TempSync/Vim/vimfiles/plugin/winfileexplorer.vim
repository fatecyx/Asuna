...
function! s:ShowDirectory()
	...
	"  --modified by boywhp 
	" Get a list of directorys and files
	let files = system("find " .b:completePath. " -maxdepth 1 \! -name \".*\" -type d")	
	let files = files . system("find " .b:completePath. " -maxdepth 1 -name \"*.[ch]\" -type f")
	let files = substitute(files, b:completePath."\n", "..\/\n", "g")
	" add filter here
	if files != "" && files !~ '\n$'
		let files = files . "\n"
	endif
